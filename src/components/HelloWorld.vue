<template>
  ...
  <div v-if="lists.length">
    <div v-for="(item, index) in lists" :key="item.msg">
      <input type="checkbox" v-model="item.done" style="cursor: pointer" />
      <span :class="{ done: item.done }">{{ item.msg }}</span>
      <span @click="deleteItem(index)" style="cursor: pointer">❎</span>
    </div>
    <div>
      <span>全选</span>
      <input type="checkbox" v-model="isAllDone" style="cursor: pointer" />
